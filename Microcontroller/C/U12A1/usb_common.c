...
void handleEP2IRQ() {				// endpoint 2, bulk in (Gerät ? Host)
	if (txBuf.len > 0) {            // Prüfen, ob Daten im Sende-Ringbuffer vorhanden sind
		while (txBuf.len > 0) {     // Solange Daten im Buffer sind (mehrere Zeichen senden erlaubt)
			uint8_t c = txBuf.buf[txBuf.out];    // Nächstes Zeichen aus dem Buffer lesen
			txBuf.out = (txBuf.out + 1) % txBufLen;  // Lesezeiger zyklisch vorwärts bewegen
			txBuf.len--;                         // Pufferlänge verringern (ein Zeichen wurde entfernt)
			USB_write_byte(c);                   // Zeichen an USB-Endpunkt senden
		}
	}
	USB_send_control_in();         // Abschlussphase für Control-IN-Transfers (evtl. nicht nötig bei BULK)
	USB_ack_fifocon();             // Daten-Transfer abschließen und FIFO freigeben (Bulk-IN abschließen)
}
